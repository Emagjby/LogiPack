<script lang="ts">
	...
	let currentOfficeLabel = $derived.by(() => {
		const office = data.profile.currentOffice;
		if (!office?.id) return notAssigned;

		const officeName =
			office.name ??
			$_("profile.office_fallback_name", {
				values: { id: office.id },
			});
		...
		<dl class="divide-y divide-surface-800">
			...
			>
				<dt
					class="text-[11px] font-medium uppercase tracking-wider text-surface-600"
				>
					{$_("profile.field.current_office")}
				</dt>
				<dd class="min-w-0 truncate text-sm text-surface-200">
					{currentOfficeLabel}
				</dd>
