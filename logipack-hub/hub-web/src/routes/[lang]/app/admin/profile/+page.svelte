		...
		<dl class="divide-y divide-surface-800">
			...
			>
				...
				<button
					...
					onclick={() => copyValue(data.profile.name, "name")}
					disabled={!data.profile.name}
					aria-label={`${$_("profile.copy")} ${$_("profile.field.name")}`}
				>
					{copiedField === "name"
						? $_("profile.copied")
						: $_("profile.copy")}
			...
			>
				...
				<button
					...
					aria-label={`${$_("profile.copy")} ${$_("profile.field.user_id")}`}
				>
					{copiedField === "userId"
						? $_("profile.copied")
						: $_("profile.copy")}
